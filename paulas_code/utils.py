...
def load_data(args):
    ...
    #nan_train = torch.isnan(target_train)
    #target_train = torch.where(nan_train, torch.tensor(0.0), target_train)
    ...
    print(input_train.min(), target_train.min(), input_val.min(), target_val.min())
    print(input_train.max(), target_train.max(), input_val.max(), target_val.max())
    #transform data
    for i in range(args.dim_channels):
...
def MSELoss_ignore_nan(output, target):
    ...
    nan = torch.isnan(target).to(device)
    target = torch.where(nan, torch.tensor(0.0).to(device), target)
    output = torch.where(nan, torch.tensor(0.0).to(device), output)
    loss = torch.square(output - target).mean()
    # loss = loss/eval_points
    #loss = np.nanmean(torch.square(output - target))
    return loss
...
def get_loss(output, true_value, in_val, args):
    if args.loss == 'mass_constraints':
        ...
        return  MSELoss_ignore_nan(output, true_value)
    else:
...
def process_for_eval(outputs, targets, mean, std, max_val, args): 
    print(outputs.min(), targets.min())
    print(outputs.max(), targets.max())
    if args.model == 'gan':
